...
**Critique generation (original v1)**: Anthropic Claude (Messages API) generates the freeform critique, given the diagram source, requirements, and retrieved corpus excerpts in the prompt context.
- *Superseded by (2026-07-12)*: Groq's OpenAI-compatible chat completions API (`llama-3.3-70b-versatile`). The maintainer didn't have Anthropic/Voyage API access; Groq offers a usable free tier and its OpenAI-compatible endpoint let the `openai` SDK replace `@anthropic-ai/sdk` with a small, isolated code change (`src/lib/review/critique.ts`).

**Retrieval / grounding (original v1)**: embed the curated corpus (a few hand-picked system design writeups, stored as markdown files in `corpus/`) using Voyage AI embeddings (pairs naturally with Anthropic, no separate account ecosystem), compute embeddings once at startup/corpus-change, and do in-process cosine-similarity top-k lookup — no dedicated vector database. The corpus is small (single-digit to low-double-digit documents) so a naive in-memory index is sufficient and avoids operating a vector store for a personal tool.
...
- *Superseded by (2026-07-12)*: no retrieval step at all — the full corpus (still just 3 pattern cards) is included in every prompt (`src/lib/corpus/index.ts`). At this corpus size, embedding-based top-k retrieval added a Voyage API dependency without meaningfully improving over "just include everything." Revisit if the corpus grows large enough that full inclusion becomes wasteful or exceeds context limits.
